...
const upsertData = async (req, res) => {
    try {
        ...
        if (slug) {
            const isSlugExist = await prisma.master_retaining_wall_condition.findFirst({ where: { slug: slug, ...(req.body.id && { NOT: { id: parseInt(req.body.id) } }) } });
            if (isSlugExist) {
        ...
        if (req.body.id) {
            ...
            await prisma.master_retaining_wall_condition.update({ where: { id: req.body.id }, data: req.body });
            ...
            await prisma.master_retaining_wall_condition.create({ data: req.body });
            await updateApiDataVersion(req.user.id);
...
const getData = async (req, res) => {
    try {
        ...
        if (page && pageSize) {
            ...
            const totalCount = await prisma.master_retaining_wall_condition.count({ where: queryFilter });
            if (skip >= totalCount) {
                skip = totalCount - pageSize;
            }
            if (skip < 0) skip = 0;
            totalPages = Math.ceil(totalCount / pageSize);
            availabilities = await prisma.master_retaining_wall_condition.findMany({
                where: queryFilter,
                skip,
                take: pageSize,
                select: {
...
const getDataById = async (req, res) => {
    try {
        ...
        const availability = await prisma.master_retaining_wall_condition.findFirst({ where: { id }, select: { id: true, name: true, slug: true } });
        if (availability) {
...
const deleteData = async (req, res) => {
    try {
        const id = parseInt(req.params.id);
        const deletedAvailability = await prisma.master_retaining_wall_condition.delete({ where: { id } });
