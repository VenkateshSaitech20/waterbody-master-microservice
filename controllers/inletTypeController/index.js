...
const upsertData = async (req, res) => {
    try {
        ...
        if (slug) {
            const isSlugExist = await prisma.master_inlet_types.findFirst({ where: { slug: slug, ...(req.body.id && { NOT: { id: parseInt(req.body.id) } }) } });
            if (isSlugExist) {
        ...
        if (req.body.id) {
            ...
            await prisma.master_inlet_types.update({ where: { id: req.body.id }, data: req.body });
            ...
            await prisma.master_inlet_types.create({ data: req.body });
            await updateApiDataVersion(req.user.id);
...
const getData = async (req, res) => {
    try {
        ...
        let totalPages;
        if (page && pageSize) {
            const totalCount = await prisma.master_inlet_types.count({ where: queryFilter });
            if (skip >= totalCount) {
                skip = totalCount - pageSize;
            }
            if (skip < 0) skip = 0;
            totalPages = Math.ceil(totalCount / pageSize);
            records = await prisma.master_inlet_types.findMany({
                where: queryFilter,
                skip,
                take: pageSize,
                select: {
...
const getDataById = async (req, res) => {
    try {
        ...
        const data = await prisma.master_inlet_types.findFirst({ where: { id }, select: { id: true, name: true, slug: true } });
        if (data) {
...
const deleteData = async (req, res) => {
    try {
        const id = parseInt(req.params.id);
        const deletedData = await prisma.master_inlet_types.delete({ where: { id } });
        if (deletedData) {
            await updateApiDataVersion(req.user.id);
